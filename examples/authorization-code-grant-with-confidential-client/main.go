...
func AuthorizationCodeExample(ctx context.Context) (*oidc.Token, error) {
	...
	authz := authenticator.Begin(conf).ClientSecret(examples.ClientConfidential, examples.ClientConfidentialSecret).AuthorizationCodeGrant("http://localhost:8081/callback", authenticator.PKCE_CODE_CHALLENGE_METHOD_S256)
